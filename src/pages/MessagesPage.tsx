    ...
    const load = async () => {
      if (isArtin) {
        const { data } = await supabase
          ...
          .select(`
            id,
            mood:moods(id, name, mood_images(url)),
            user:profiles!matches_user_id_fkey(id, name, profile_images(url))
          `)
    ...
    <div className="h-full overflow-y-auto">
      <div className="divide-y divide-gray-50">
        {matches.map(match => {
          ...
          const avatarUrl = isArtin
            ? (userImages[0]?.url || null)
            : (moodImages[0]?.url || null)
          const title = isArtin
            ? `${match.user?.name} → ${match.mood?.name}`
            : match.mood?.name
