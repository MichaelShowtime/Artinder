...
export default function EditMoodPage() {
  ...
  const [existingImages, setExistingImages] = useState<MoodImage[]>([])
  ...
  useEffect(() => {
    if (!moodId) return
    supabase.from('moods').select('*, mood_images(*)').eq('id', moodId).single().then(({ data }) => {
      if (data) {
        setName(data.name)
        setDescription(data.description || '')
        setTags(data.tags || [])
        setPrompts(data.prompts || [])
        setExistingImages((data.mood_images as MoodImage[]) || [])
      }
    })
  }, [moodId])
  ...
  const removeExisting = async (img: MoodImage) => {
    await supabase.from('mood_images').delete().eq('id', img.id)
  ...
  const save = async () => {
    if (!name.trim() || !moodId) return
    setSaving(true)
    setError('')
    try {
      await supabase.from('moods').update({ name, description, tags, prompts }).eq('id', moodId)
      for (let i = 0; i < newPhotos.length; i++) {
        const file = newPhotos[i]
        const ext = file.name.split('.').pop()
        const path = `moods/${moodId}/${Date.now()}_${i}.${ext}`
        ...
        if (up) {
          ...
          await supabase.from('mood_images').insert({ mood_id: moodId, url: publicUrl, order_index: existingImages.length + i })
        }
      }
      navigate('/artin')
    } catch (_err) {
          ...
          <div className="grid grid-cols-3 gap-2">
            {existingImages.map(img => (
              <div key={img.id} className="relative aspect-square">
                <img src={img.url} className="w-full h-full object-cover rounded-xl" alt="Persona" />
                <button onClick={() => removeExisting(img)} className="absolute top-1 right-1 w-6 h-6 bg-black/50 rounded-full flex items-center justify-center">
            ...
            {newPreviews.map((p, i) => (
              <div key={`new-${i}`} className="relative aspect-square">
                <img src={p} className="w-full h-full object-cover rounded-xl" alt={`Ny ${i}`} />
                <button onClick={() => removeNew(i)} className="absolute top-1 right-1 w-6 h-6 bg-black/50 rounded-full flex items-center justify-center">
                  <X size={12} className="text-white" />
                </button>
              </div>
            ))}
